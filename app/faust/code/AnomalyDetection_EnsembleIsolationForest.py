...
async def anomaly_detection_agent(stream):
    ...
    async for event in stream:
        # Parse the incoming event to create a DataFrame row
        ...
        # Initialize columns and train initial Isolation Forest model
        ...
        # Detect anomalies
        scores, predictions = detect_anomalies(isolation_forest_model, row_df[numeric_columns])
        row_df['scores'] = scores
        row_df['anomaly'] = ['yes' if pred == -1 else 'no' for pred in predictions]
        
        # Append row to received data and update row count
        ...
        # Periodically update the Isolation Forest model every `update_interval` rows
        if row_count % update_interval == 0:
            recent_data = received_data.iloc[-initial_block_size:]
            isolation_forest_model = train_isolation_forest(recent_data[numeric_columns])
            print(f"Isolation Forest model updated after {row_count} rows.")

        # Save to CSV in batches of 100 rows
        if row_count % 100 == 0:
